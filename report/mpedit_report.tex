...
\STATE $\mathbf{c}_0 \gets \text{PathToCtrlPts}(\tau_\text{ref})$
\STATE $\mathbf{c}_0 \gets \text{Normalize}(\mathbf{c}_0)$
\FOR{$i = 1$ \TO $N$}
...
\FOR{$t = t_0, t_0{-}1, \dots, 1$}
  \STATE $\hat\epsilon \gets \epsilon_\theta(\mathbf{c}_t^{(1{:}N)}, t, \mathbf{z})$  \hfill $\triangleright$ \textit{batched}
  \STATE $\mathbf{c}_{t-1}^{(1{:}N)} \gets \text{DDIMStep}(\mathbf{c}_t^{(1{:}N)}, \hat\epsilon)
         - \eta\,\nabla_{\mathbf{c}_t}\mathcal{J}_\text{coll}}$
  \STATE Clamp start/goal control points
\ENDFOR
\STATE $\mathbf{c}_0^{(1{:}N)} \gets \text{Unnormalize}(\mathbf{c}_0^{(1{:}N)})$
\STATE $\mathcal{T} \gets \{\tau^{(i)} \mid \tau^{(i)}
       = \text{BSpline}(\mathbf{c}_0^{(i)}),\;
       \text{collision-free}\}$
\RETURN $\arg\min_{\tau\in\mathcal{T}} \text{PathLength}(\tau)$
\end{algorithmic}
\end{algorithm}
...
\begin{itemize}
  ...
  \item \textbf{Full MPD:} standard MPD inference from pure noise ($t_0{=}T$,
        i.e.\ the full diffusion schedule starting from random noise).
...
\begin{figure}[t]
  \centering
  % Replace with: \includegraphics[width=\linewidth]{../scripts/inference/logs_experiments/figures/fig2_t0_sweep.pdf}
...
\begin{figure}[t]
  \centering
  % Replace with: \includegraphics[width=0.9\linewidth]{../scripts/inference/logs_experiments/figures/fig_baselines_comparison.pdf}
...
\begin{figure}[t]
  \centering
  % Replace with: \includegraphics[width=\linewidth]{../scripts/inference/logs_experiments/figures/fig5_speed.pdf}
...
\begin{figure}[t]
  \centering
  % Replace with: \includegraphics[width=\linewidth]{../scripts/inference/logs_experiments/figures/fig6_diversity.pdf}
...
\begin{figure}[H]
  \centering
  % Replace with: \includegraphics[width=\linewidth]{../scripts/inference/logs_experiments/figures/fig4_sketch_heatmap.pdf}
